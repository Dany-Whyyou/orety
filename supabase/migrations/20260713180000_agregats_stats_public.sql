-- Le schéma `orety` n'est pas exposé par PostgREST (seuls public et graphql_public
-- le sont). La fonction d'agrégats doit donc vivre dans `public` pour être
-- appelable en RPC. Elle reste security definer et n'est accordée qu'au service role.

drop function if exists orety.stats_agregees(uuid[]);
...
comment on function public.stats_agregees(uuid[]) is
